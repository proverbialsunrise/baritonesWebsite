    ...
    if(!err) {
      ...
      if(!err) {
        ...
        for (var i = 0; i < numberOfEvents; i++) {
         ...
         err = "Calendar data was unparseable. Try again later.";
         eventsCallback(events, err);
       }
      } else {
        eventsCallback(events, err);
...
function dateFormatTwo(summary) {
  var dateString = getDateStringForEventWithTime(summary);
  if (dateString) {
    var theDate = moment(dateString, 'D MMM YYYY hh:mm', true);
    if (theDate.isValid()) {
      theDate.hasTime = true;
      return theDate;
    }
...
function dateFormatThree(summary) {
  ...
  if (dateString) {
    var theDate = moment(dateString, 'D MMM YYYY', true);
    if (theDate.isValid()) {
...
function dateFormatFour(summary) {
  var dateString = getDateStringForEventWithoutTime(summary);
  if (dateString) {
    var theDate = moment(dateString, 'MMM DD, YYYY', true);
    if (theDate.isValid()) {
      theDate.hasTime = false;
      return theDate;
    }
...
function getDateFromSummary(summary) {
  var getDateFormats = [dateFormatOne, dateFormatTwo, dateFormatThree, dateFormatFour];
